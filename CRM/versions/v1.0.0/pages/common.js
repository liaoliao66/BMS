const NAV_ITEMS = [
  { href: '工作台_数据概览_page.html', icon: 'fa-gauge-high', label: '工作台' },
  { href: '客户管理_客户列表_page.html', icon: 'fa-building', label: '客户管理' },
  { href: '商机管理_商机列表_page.html', icon: 'fa-chart-line', label: '商机管理' },
  { href: '合同管理_合同列表_page.html', icon: 'fa-file-contract', label: '合同管理' },
  { href: '跟进记录_跟进列表_page.html', icon: 'fa-comments', label: '跟进记录' }
];
...
function renderSidebar(active) {
  const items = NAV_ITEMS.map(n => {
    ...
    return `<a href="${n.href}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition ${isActive ? 'bg-teal-600/20 text-teal-300' : 'text-slate-300 hover:bg-slate-700 hover:text-white'}">
      <i class="fa-solid ${n.icon} w-5 text-center"></i>${n.label}
    </a>`;
  }).join('');
  return `<aside class="fixed left-0 top-0 h-full w-56 bg-slate-800 text-white flex flex-col z-20 shadow-xl">
    <div class="p-5 border-b border-slate-700">
      <div class="flex items-center gap-2"><i class="fa-solid fa-anchor text-teal-400 text-lg"></i><span class="font-semibold">铁水联运码头 CRM</span></div>
      <p class="text-xs text-slate-400 mt-1">市场部 · PC 端 · v1.0.0</p>
    </div>
    <nav class="flex-1 p-3 space-y-1">${items}</nav>
    <div class="p-4 border-t border-slate-700 text-xs text-slate-500">纯 CRM · 不含审批/消息/权限</div>
  </aside>`;
}
...
function initPage(activeNav) {
  ...
  if (mount) mount.innerHTML = renderSidebar(activeNav);
  openModalByParam();
}
