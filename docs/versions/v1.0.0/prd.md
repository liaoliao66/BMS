...
| 当前版本 | v1.0.0 |
| 版本更新时间 | 2026-08-25 |
| 版本迭代说明 | 新增一级菜单「合同收款」：合同管理、收款管理；客户选自 CRM |
...
**v1.0.0 以「目标 → 计划 → 任务 → 汇报填报 → 领导台账」闭环为主链路**；分部门、分**专项**管理；目标含介绍、截止时间与可改状态（改状态必填原因）；任务中心含我的任务、任务督办、汇报提醒；不建设独立审批/消息/权限配置页面。
...
**文字解读：** 填报以任务为单元、以周/月为周期切片；提交后向上汇总到计划与目标。
...
**文字解读：** 数据由上而下下达、由下而上汇总；看板只读展示汇总结果。
...
| 维度 | 内容 |
| --- | --- |
| 功能介绍 | 一屏掌握公司经营：目标完成、计划/任务链路、部门达成、专项进展、汇报催填、督办与**合同回款**摘要 |
...
| 区块 | 说明 | 优先级 |
| --- | --- | --- |
| 摘要 KPI | 目标完成率、进行中目标/计划、待汇报、风险滞后、督办中、合同回款率、剩余收款 | P0 |
| 工作汇报快照 | 本周/本月较上期一句对比 + 链到工作汇报 | P0 |
| 目标·计划·任务链路 | 年/季/月目标、计划、任务数量与状态摘要 | P0 |
| 合同收款摘要 | 合同总数/总额/回款/剩余；回款进度条；链到合同/收款管理 | P0 |
| 各部门达成 | 部门完成率卡片，下钻弹窗含目标/计划/滞后任务 | P0 |
| 重点专项 | 专项完成率列表，下钻含关联计划与关键任务 | P0 |
...
| 必显列 | 目标名称、类型、周期、责任部门、完成率、状态、操作 | P0 |
...
| 维度 | 内容 |
| --- | --- |
| 功能介绍 | 大尺寸详情弹窗：展示目标类型、上级路径、介绍、截止、状态；Tab 含**下级目标**、关联计划、关联任务、状态变更 |
| 前置条件 | 目标已存在 |
| 数据权限 | 同目标可见范围 |
...
| 维度 | 内容 |
| --- | --- |
| 功能介绍 | 大弹窗展示计划说明、起止时间、关键节点列表、专项、附件、下属任务；可下钻任务 |
| 前置条件 | 计划已存在 |
| 数据权限 | 同计划可见范围 |
...
| 数据权限 | 领导看全公司；部门负责人看本部门 |
| 页面跳转 | 人员行可下钻任务详情；确认提醒弹窗后由平台消息触达 |
...
| 维度 | 内容 |
| --- | --- |
| 功能介绍 | 菜单名「工作汇报」；单页 Tab 切换**周报 / 月报**；汇总展示目标完成、计划进展、任务落地；摘要含较上期对比 |
| 前置条件 | 周期结束生成快照；任务汇报数据已沉淀 |
| 数据权限 | 领导、部门负责人全范围；执行人可看本人相关 |
| 页面跳转 | 菜单归属**经营台账**；未填催填链到「汇报提醒」；下钻目标/计划/任务已有详情弹窗 |
...
| UF-04 | 执行人员 | 我的任务 → 选待汇报 → 进展弹窗提交 → 任务详情发表跟踪评论 | 台账与时间线更新 |
...
| 目标值 | 数值或文本目标 | 是 | P0 |
| 当前值/完成率 | 展示用；首版不做下级强制加总 | 系统/手工 | P0 |
| 状态 | 草稿/进行中/已完成/已延期/已取消 | 是 | P0 |
| 状态变更原因 | 每次手工改状态时必填 | 改状态时必填 | P0 |
...
| 口径 | 规则建议 | 优先级 |
| --- | --- | --- |
| 目标完成率 | 下属任务完成率加权平均；无任务则显示未分解 | P0 |
| 滞后 | 当前日期 > 结束日期且状态≠已完成；或连续 2 个填报周期未更新 | P0 |
...
| 风险 | 影响 | 缓解建议 | 优先级 |
| --- | --- | --- |
| 目标-计划-任务层级过深导致填报负担重 | 执行人不愿更新，看板失真 | 首版强制周/月必填最小字段；待填报中心聚合入口 | P0 |
| 完成率口径不统一 | 领导对数字不信任 | 评审锁定汇总公式后再开发 | P0 |
...
| Q3 | 完成率：纯手工填报，还是任务自动汇总到目标 | 看板口径 | 任务汇总 + 允许目标层手动校正 |
...
| 一级分组 | 二级菜单 | 说明 | 优先级 |
| --- | --- | --- | --- |
| 经营台账 | 经营总览 | 公司级总览：KPI、链路、部门/专项、风险待办、合同回款摘要 | P0 |
| 经营台账 | 工作台 | 本部门目标 / 计划 / 任务（原「部门工作台」）；紧随经营总览 | P0 |
| 经营台账 | 工作汇报 | 周报/月报周期汇总：目标、计划、任务；较上期对比 | P0 |
...
| P2 | 更细指标字典、个人目标、年/季专用可视化视图、多主责目标 |
