...
| 当前版本 | v1.0.0 |
| 版本更新时间 | 2026-08-27 |
| 版本迭代说明 | **合同收款重构** + **经营总览 KPI 分组**（合同全量 / 任务随筛选）；发票作废仅改状态、开票与收款无强关联；产品更名、专项管理等见历史迭代 |
...
| 收款记录 | 必须关联合同登记实际收款；关联开票选填（无强关联）；统计本年计划收款（独立配置）、累计收款、剩余收款 | P0 |
| 开票记录 | 必须关联合同登记已开发票；支持作废（仅改状态）；统计累计开票、待开票、开票笔数 | P0 |
...
| 页面跳转 | 部门卡片 → 部门详情弹窗 → 工作台；风险/待汇报 → 任务详情/汇报提醒/任务督办；合同摘要 → 合同/开票/收款记录；重点专项 → **专项管理详情** |

**页面区块（P0）：**

| 区块 | 说明 | 优先级 |
| --- | --- | --- |
| **合同·开票·收款区（全量）** | 独立分组展示；**不随年度/季度筛选变化**；含回款率、累计开票、剩余收款 KPI 及明细/进度条 | P0 |
| **目标·计划·任务区（随筛选）** | 独立分组；筛选器（年/季/部门）**仅作用于本区**；含需关注目标、节点、计划、待汇报、风险、督办 KPI | P0 |
| 目标·计划·任务链路 | 年/季/月目标、计划、任务数量与状态摘要（随筛选变化） | P0 |
| 各部门达成 | 部门**健康分布**卡片（正常 X · 需关注 Y · 风险 Z），下钻弹窗含目标/计划/滞后任务（随筛选变化） | P0 |
| 重点专项 | 专项**节点达成 + 滞后任务**列表；点击跳转**专项管理详情大弹窗**（含关联计划与任务） | P0 |
| 风险与待办 | 滞后任务清单 + 待汇报人员清单（链汇报提醒；随筛选变化） | P0 |

**筛选与口径规则（P0）：**

| 规则 | 说明 |
| --- | --- |
| 分组展示 | 首屏 KPI **分两组**：① 合同/开票/收款（全量累计）；② 目标/计划/任务（经营执行） |
| 时间筛选范围 | 年度、季度、部门筛选**仅影响第二组**及下属链路、部门达成、风险待办 |
| 合同区口径 | 合同总数/总额、累计开票、回款、剩余收款等为**全量累计**，不受 Q1/Q2 等切换影响 |
| 任务区口径 | 需关注目标、进行中计划/任务、待汇报、督办等按所选**年/季/部门**聚合 |

> **已移除（v1.0.0）：** 经营总览页「工作汇报快照」横幅区块；周期快照仅在**工作周报**页查看（见 §4.7.2）。

**正常流程：** 扫合同全量区掌握收付 → 切换年/季/部门看经营执行 KPI → 下钻部门或专项 → 跳转工作台/汇报提醒/合同收款/专项管理跟进。  
**边界情况：** 无合同时合同区空态；部门负责人文案仅本部门；切换筛选时合同区数值保持不变。  
...
**正常流程：** 选关联计划 → 填督办内容 → 选责任人 →（可选）添加图片 → 下达 → 对方「我的任务」可见。  
...
**适用入口（P0）：** 我的任务新建/编辑、任务列表新建/编辑、任务督办下达、部门工作台「新增任务」。

#### 4.4.5 任务详情与周期汇报

| 维度 | 内容 |
| --- | --- |
| 功能介绍 | 查看任务全貌，提交年/季/月/周进展；**不含完成率字段**；含**转派记录**时间线 |
| 前置条件 | 任务进行中或草稿已发布 |
| 数据权限 | 责任人可汇报；上级可查看 |
...
```mermaid
flowchart TD
  A["任务清单/详情 · 转派"] --> B{进行中或已延期?}
  B -->|否| X["拦截：已完成/已取消不可转派"]
  B -->|是| C[选新责任人 + 填原因]
  C --> D{校验}
  D -->|失败| E["未选人/选本人/原因不足/非本部门"]
  D -->|通过| F[写入转派记录]
  F --> G[更新责任人]
  G --> H[消息通知新旧责任人]
  H --> I[刷新列表/详情]
...
> **专项业务状态不含「需关注」**（需关注为**目标健康状态**口径，不用于专项档案状态）。
...
| 前置条件 | 专项已存在 |
| 数据权限 | 同专项可见范围 |
| 页面跳转 | 关联计划名称 → 计划管理详情；任务名称 → 任务详情；经营总览「重点专项」可深链打开本弹窗 |
...
**业务链路（P0）：** 合同建档 → **开票记录**（登记已开发票）→ **收款记录**（登记实际到账）。开票与收款均**必须关联合同**；**开票与收款之间无强关联**（收款可不关联开票，无强制先后顺序）。

#### 4.8.1 合同管理

| 维度 | 内容 |
| --- | --- |
| 功能介绍 | 录入已签订合同；列表展示并统计**合同总数、合同总额、累计开票、回款总额、剩余收款** |
| 前置条件 | 客户主数据已在 **CRM** 维护；本模块仅选择客户，不维护客户档案 |
| 数据权限 | 商务/财务相关角色可写；领导可看汇总 |
| 页面跳转 | 新建/编辑/详情大弹窗；可跳转**开票记录**登记开票、**收款记录**登记收款 |
...
| 合同总数 | 未作废合同条数 |
| 合同总额 | 合同金额合计 |
| 累计开票 | 状态为「已开票」的开票记录合计（**不含已作废**） |
| 回款总额 | 关联收款记录合计 |
| 剩余收款 | 合同总额 − 回款总额 |
| 待开票 | 合同总额 − 累计开票（按合同维度汇总；不含已作废发票） |
...
| 超额开票 | 允许；保存时提示「累计开票将超过合同金额」 | P0 |

**正常流程：** 选客户 → 填合同信息 → 保存 → 按需登记开票/收款（顺序不限）→ 摘要刷新。  
**边界情况：** 尚无开票/收款时对应指标为 0；允许先收款后开票、预收款不关联开票；已作废发票不计入累计开票。  
**异常兜底：** 编号重复、金额非法拦截。
...
| 维度 | 内容 |
| --- | --- |
| 功能介绍 | **必须关联合同**登记已开发票；记录发票号码、开票金额、开票日期、发票类型等 |
| 前置条件 | 至少存在一条合同 |
| 数据权限 | 同合同管理可写范围 |
| 页面跳转 | 登记开票弹窗；合同行/详情可带入合同；可跳转收款记录登记收款 |

**列表必显列（P0）：** 开票日期、发票号码、关联合同、客户、开票金额、发票类型、状态、操作（编辑 / 作废 / 登记收款；已作废仅展示不可操作）。

**摘要口径（P0）：**

| 指标 | 口径 |
| --- | --- |
| 累计开票 | 状态为「已开票」的记录合计（**不含已作废**；可按年筛选） |
| 本月开票 | 当前自然月有效开票合计 |
| 待开票 | 合同总额合计 − 累计开票 |
| 开票笔数 | 有效开票记录条数（不含已作废） |
...
| --- | --- | --- |
| 关联合同必选 | 未选合同不可保存 | P0 |
| 必填项 | 发票号码、开票金额、开票日期、发票类型 | P0 |
| 发票类型 | 增值税专用发票 / 增值税普通发票 | P0 |
| 超额提示 | 单笔导致合同累计开票 > 合同金额时强提示，仍可确认保存 | P0 |
| 发票作废 | 列表操作「作废」→ 二次确认 → **仅将状态标记为「已作废」**；不涉及红冲流程；已作废不计入累计开票/待开票 | P0 |

**正常流程：** 选合同 → 填发票信息与金额 → 保存 → 合同「已开票/待开票」更新；需作废时在列表操作标记状态。  
**边界情况：** 一合同多张发票；已作废发票保留记录、列表可筛「已作废」、不可再编辑或重复作废。  
**异常兜底：** 未选合同、金额≤0、缺发票号拦截。

#### 4.8.3 收款记录（原「收款管理」· P0）

| 维度 | 内容 |
| --- | --- |
| 功能介绍 | **必须关联合同**登记实际收款；**关联开票为选填**（开票与收款无强关联）；统计**本年计划收款、累计开票、累计收款、剩余收款** |
| 前置条件 | 至少存在一条合同 |
| 数据权限 | 同合同管理可写范围 |
| 页面跳转 | 登记收款弹窗；合同/开票行可带入合同；可回看合同详情 |
...
| --- | --- | --- |
| 关联合同必选 | 未选合同不可保存 | P0 |
| 关联开票 | **选填**；下拉展示该合同下状态为「已开票」的发票；**不强制与开票绑定**，支持预收款、先收款后开票 | P1 |
| 本年计划独立配置 | 单独维护「本年度计划收款额」，不从合同汇总 | P0 |
| 超额提示 | 单笔导致合同累计回款 > 合同金额时强提示，仍可确认保存 | P0 |

**正常流程：** 配置本年计划（可选）→ 选合同登记收款（关联开票可不选）→ 合同回款/剩余更新。  
**边界情况：** 一合同多笔收款；预收款/先收款后开票时关联开票为空；开票作废后已关联的收款记录保留、不做级联处理。  
**异常兜底：** 未选合同、金额≤0 拦截。
...
| UF-09 | 商务/财务 | 合同管理 → 选 CRM 客户 → 录入合同 → 保存 | 摘要含开票/回款指标 |
| UF-10 | 商务/财务 | 开票记录 → 选合同登记开票 → 保存 | 累计/待开票更新；超额强提示 |
| UF-11 | 商务/财务 | 收款记录 → 配置本年计划（独立）→ 选合同（可选关联发票）登记收款 → 保存 | 累计/剩余更新；超额强提示 |
...
| 开票记录 | /invoices | P0 | 商务/财务、领导 |
| 收款记录 | /receipts | P0 | 商务/财务、领导 |
...
> **说明：** 「需关注」仅用于**目标健康状态**（§7.10.1），不作为专项业务状态枚举值。
...
| 合同说明 / 附件 | — | 否 | P1 |
| 回款总额 / 剩余收款 | 系统按收款记录汇总 | 系统 | P0 |
| 累计开票 / 待开票 | 系统按开票记录汇总 | 系统 | P0 |
...
| 开票金额 | > 0，单位万元 | 是 | P0 |
| 开票日期 | — | 是 | P0 |
| 发票类型 | 增值税专用发票 / 增值税普通发票 | 是 | P0 |
| 状态 | 已开票 / 已作废；新建默认为「已开票」 | 是 | P0 |
| 备注 | — | 否 | P1 |
...
| --- | --- | --- | --- |
| 关联合同 | 必须选择 | 是 | P0 |
| 关联开票 | 选填；该合同下状态为「已开票」的发票；可不选 | 否 | P1 |
| 收款金额 | > 0 | 是 | P0 |
| 收款日期 | — | 是 | P0 |
| 收款方式 | 银行转账等 | 否 | P1 |
| 说明 / 凭证附件 | — | 否 | P1 |
...
| KPI | 规则 | 优先级 |
| --- | --- | --- |
| **分组规则** | KPI 分两组：**合同/开票/收款**（全量，不随年/季变）与**目标/计划/任务**（随年/季/部门筛选变） | P0 |
...
| 累计开票 | 状态为「已开票」的开票记录合计（**不含已作废**）；经营总览 KPI 卡与摘要区展示 | P0 |
| 待开票 | 合同总额 − 累计开票（不含已作废） | P0 |
...
| Q18 | 经营总览 KPI 保留项 | 经营总览 | **已确认**：需关注目标、滞后任务、待汇报、本周到期节点、合同回款、**累计开票**；不含工作汇报快照条 |
| I1 | 收款管理菜单更名 | 合同收款 | **已确认**：更名为「收款记录」 |
| I2 | 是否新增开票记录 | 合同收款 | **已确认**：新增独立菜单与页面；必须关联合同 |
| I3 | 发票作废规则 | 开票记录 | **已确认**：仅标记状态为「已作废」；不计入累计开票/待开票；无红冲流程 |
| I4 | 开票与收款是否强关联 | 合同收款 | **已确认**：无强关联；收款可不关联开票；无强制先后顺序 |
| I5 | 经营总览 KPI 是否分组 | 经营总览 | **已确认**：合同/收付款全量一组；目标/计划/任务随年/季/部门筛选一组 |
| P1 | 经营总览是否展示工作汇报快照条 | 经营总览 | **已确认**：不展示；快照仅在工作周报页 |
| P2 | 专项列表状态筛选形态 | 专项管理 | **已确认**：页签（全部/进行中/已延期/已完成），非下拉 |
| P3 | 专项业务状态是否含「需关注」 | 专项管理 | **已确认**：不含；需关注仅用于目标健康状态 |
...
| 一级分组 | 二级菜单 | 说明 | 优先级 |
| --- | --- | --- | --- |
| 经营台账 | 经营总览 | 公司级总览：需关注目标、节点到期、部门健康、专项、风险待办、合同/开票/收款摘要 | P0 |
...
| 专项 | 专项管理 | 列表（状态页签）+ 新建/编辑/详情大弹窗 + 删除确认（级联关联数据） | P0 |
| 合同收款 | 合同管理 | 合同录入；总数/总额/开票/回款/剩余 | P0 |
| 合同收款 | 开票记录 | 关联合同登记开票；累计/待开票 | P0 |
| 合同收款 | 收款记录 | 关联合同收款；可选关联开票；本年计划独立配置 | P0 |
